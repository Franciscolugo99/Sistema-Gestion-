...
$results[] = flus_run_test('movimientos stock quedan versionados y respetan stock anterior', function (): void {
    ...
    $stockConsultaPhp = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'stock_consulta.php');
    $productosConsultaPhp = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'productos_consulta.php');
    ...
    flus_assert_contains('data-stock-refresh', $stockPhp);
    flus_assert_contains('data-stock-adjust', $stockPhp);
    flus_assert_contains('data-stock-close-modal', $stockPhp);
    flus_assert_contains('data-stock-confirm-adjust', $stockPhp);
    flus_assert_contains('adjustSubmitting: false', $stockJs);
    flus_assert_contains('quickAdjustFromButton(button)', $stockJs);
    flus_assert_contains("document.querySelector('[data-stock-confirm-adjust]')", $stockJs);
    flus_assert_contains("ANULACION_COMPRA: 'Anulación de compra'", $stockJs);
    flus_assert_contains("ANULACION_VENTA: 'Anulación de venta'", $stockJs);
    ...
    flus_assert_contains('.stock-col-code', $stockCss);
...
$results[] = flus_run_test('pulido operativo usa solo acciones disponibles y feedback FLUS en usuarios', function (): void {
    $repoRoot = dirname(__DIR__);
    $stockPhp = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'stock.php');
    $usuariosPhp = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'usuarios.php');
    ...
    flus_assert_contains('data-users-clear-search', $usuariosPhp);
    flus_assert_contains('data-users-autosubmit', $usuariosPhp);
    flus_assert_contains('[data-users-autosubmit]', $usuariosJs);
    flus_assert_contains('[data-users-clear-search]', $usuariosJs);
    ...
    flus_assert_not_contains('onclick=', $usuariosPhp);
    flus_assert_not_contains('onchange=', $usuariosPhp);
...
$results[] = flus_run_test('clientes proveedores y roles confirman acciones sensibles sin dialogos nativos', function (): void {
    ...
    flus_assert_contains('data-role-drawer-open', $rolesPhp);
    flus_assert_contains('data-role-search', $rolesPhp);
    flus_assert_contains('data-role-name-input', $rolesPhp);
    flus_assert_contains('data-role-slug-input', $rolesPhp);
    flus_assert_contains('data-role-progress=', $rolesPhp);
    flus_assert_contains('role-critical-alert is-hidden', $rolesPhp);
    flus_assert_contains('[data-role-drawer-open]', $rolesJs);
    flus_assert_contains('[data-role-progress]', $rolesJs);
    flus_assert_contains("setCriticalRoleAlertVisible(true);", $rolesJs);
    flus_assert_contains('.role-card.is-hidden', $rolesCss);
    flus_assert_contains('.role-critical-alert.is-hidden', $rolesCss);
    flus_assert_not_contains('onclick=', $rolesPhp);
    flus_assert_not_contains('onkeyup=', $rolesPhp);
    flus_assert_not_contains('oninput=', $rolesPhp);
    flus_assert_not_contains('style=', $rolesPhp);
    flus_assert_not_contains('card.style.display', $rolesJs);
    ...
    flus_assert_contains('data-role-filter="operativo"', $permisosPhp);
    flus_assert_contains('data-role-category-toggle', $permisosPhp);
    flus_assert_contains('[data-role-filter]', $permisosJs);
    flus_assert_contains('[data-role-expand-all]', $permisosJs);
    flus_assert_contains('.permisos-category.is-hidden', $rolesCss);
    flus_assert_not_contains('onclick=', $permisosPhp);
    flus_assert_not_contains('category.style.display', $permisosJs);
...
$results[] = flus_run_test('historial de precios conserva scroll de pagina y aplica filtros sin recargas intermedias', function (): void {
    $repoRoot = dirname(__DIR__);
    $preciosPhp = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'precios_historial.php');
    $preciosJs = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'js' . DIRECTORY_SEPARATOR . 'precios.js');
...
$results[] = flus_run_test('install baseline includes core POS sale tables', function (): void {
    ...
    flus_assert_contains("latest migration is 046", $integrationPhp);
...
$results[] = flus_run_test('release 4.2.10 alinea version docs y comandos remotos', function (): void {
    $repoRoot = dirname(__DIR__);
    ...
    $releaseDoc = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'docs' . DIRECTORY_SEPARATOR . 'RELEASE_4_2_10.md');
    ...
    flus_assert_contains("define('FLUS_VERSION', '4.2.10')", $versionPhp);
    flus_assert_contains("define('FLUS_BUILD',   '2026-07-30')", $versionPhp);
    flus_assert_contains('**Version:** 4.2.10', $readme);
    flus_assert_contains('**Build:** 2026-07-30', $readme);
    flus_assert_contains('la rama conserva el nombre historico `Ver-4.0.0`', $readme);
    flus_assert_contains('## [4.2.10] - 2026-07-30', $changelog);
    flus_assert_contains('Version visible: `4.2.10`', $releaseDoc);
    flus_assert_contains('FLUS_Server_Setup_4.2.10.exe', $releaseDoc);
...
$results[] = flus_run_test('cuenta corriente actions salen del switch y usan guard central', function (): void {
    ...
    flus_assert_contains('value="CREDITO"', $ccPhp);
    flus_assert_contains('value="MODO"', $ccPhp);
    flus_assert_contains('value="QR"', $ccPhp);
    flus_assert_contains('value="CREDITO"', $ccClientePhp);
    flus_assert_contains('value="MODO"', $ccClientePhp);
    flus_assert_contains('value="QR"', $ccClientePhp);
    flus_assert_contains('id="formAjuste" class="drawer-body"', $ccClientePhp);
    flus_assert_contains('class="drawer-close" id="btnCerrarDrawerAjuste"', $ccClientePhp);
    flus_assert_contains('.ajuste-tipo-grid', $ccCss);
    flus_assert_contains('.cliente-mini-info', $ccCss);
    flus_assert_contains('.cc-inline-note', $ccCss);
    flus_assert_contains('grid-template-columns: repeat(2, minmax(0, 1fr));', $ccCss);
    flus_assert_not_contains('onchange="this.form.submit()"', $ccPhp);
    flus_assert_not_contains('onchange="this.form.submit()"', $ccClientePhp);
    flus_assert_not_contains("</div>\r\n    </div>\r\n    <label class=\"medio-pago-option\">", $ccPhp);
    flus_assert_not_contains('style="display:flex"', $ccClientePhp);
    flus_assert_not_contains('style="font-size:.82em"', $ccClientePhp);
    flus_assert_not_contains('class="btn-close" id="btnCerrarDrawerAjuste"', $ccClientePhp);
...
$results[] = flus_run_test('cloud sync local encola ventas sin bloquear la operacion', function (): void {
    ...
    $commandMigrationSql = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'migrations' . DIRECTORY_SEPARATOR . '046_cloud_command_receipts.sql');
    $registrarVentaPhp = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'api' . DIRECTORY_SEPARATOR . 'actions' . DIRECTORY_SEPARATOR . 'registrar_venta.php');
    $anularVentaPhp = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'api' . DIRECTORY_SEPARATOR . 'actions' . DIRECTORY_SEPARATOR . 'anular_venta.php');
    $anularItemsPhp = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'api' . DIRECTORY_SEPARATOR . 'actions' . DIRECTORY_SEPARATOR . 'anular_items_venta.php');
    $cajaPhp = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'caja.php');
    $cajaCerrarPhp = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'caja_cerrar.php');
    $indexPhp = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'api' . DIRECTORY_SEPARATOR . 'index.php');
    $cloudLibPhp = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR . 'cloud_sync_lib.php');
    $commandLibPhp = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR . 'cloud_command_lib.php');
    $tecnicoPhp = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'tecnico.php');
    ...
    $salesBackfillPhp = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'scripts' . DIRECTORY_SEPARATOR . 'cloud_sync_sales_backfill.php');
    $salesBackfillLibPhp = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR . 'cloud_sync_sales_backfill_lib.php');
    ...
    flus_assert_contains('CREATE TABLE `cloud_command_receipts`', $installSql);
    flus_assert_contains('CREATE TABLE IF NOT EXISTS `cloud_command_receipts`', $commandMigrationSql);
    flus_assert_contains('UNIQUE KEY `ux_cloud_command_receipts_uid` (`command_uid`)', $commandMigrationSql);
    ...
    flus_assert_contains("'cajero_nombre' => \$cajeroNombre", $registrarVentaPhp);
    ...
    flus_assert_contains("'cajero_nombre' => \$cashierName", $cloudLibPhp);
    flus_assert_contains('function flus_cloud_sync_enqueue_sale_annulment(PDO $pdo, array $annulment): array', $cloudLibPhp);
    flus_assert_contains("return flus_cloud_sync_enqueue_event(\$pdo, 'sale.annulled'", $cloudLibPhp);
    flus_assert_contains('flus_cloud_sync_enqueue_sale_annulment($pdo, [', $anularVentaPhp);
    flus_assert_contains('flus_cloud_sync_enqueue_sale_annulment($pdo, [', $anularItemsPhp);
    flus_assert_contains("array_key_exists('enqueue', \$options)", $salesBackfillPhp);
    flus_assert_contains('flus_cloud_sync_sales_backfill($pdo, $backfillOptions, $enqueue)', $salesBackfillPhp);
    flus_assert_not_contains('flus_cloud_sync_push(', $salesBackfillPhp);
    flus_assert_contains('function flus_cloud_sync_sales_backfill(PDO $pdo, array $options, bool $enqueue = false): array', $salesBackfillLibPhp);
    flus_assert_contains("'sale-id:' . \$saleId", $salesBackfillLibPhp);
    flus_assert_contains('flus_cloud_sync_enqueue_sale($pdo, [', $salesBackfillLibPhp);
    flus_assert_not_contains('flus_cloud_sync_push(', $salesBackfillLibPhp);
    ...
    flus_assert_contains("require_once FLUS_ROOT . '/src/cloud_sync_sales_backfill_lib.php';", $tecnicoPhp);
    flus_assert_contains("require_permission('administrar_config');", $tecnicoPhp);
    flus_assert_contains('name="csrf_token"', $tecnicoPhp);
    flus_assert_contains('cloud_sales_backfill_preview', $tecnicoPhp);
    flus_assert_contains('cloud_sales_backfill_enqueue', $tecnicoPhp);
    flus_assert_contains('sin envio inmediato', $tecnicoPhp);
    ...
    flus_assert_contains("require_once \$root . '/src/cloud_command_lib.php';", $tickScriptPhp);
    flus_assert_contains('flus_cloud_commands_run($pdo, 5)', $tickScriptPhp);
    flus_assert_contains("'commands' => \$commands", $tickScriptPhp);
    ...
    flus_assert_contains('function flus_cloud_command_process(PDO $pdo, array $command): array', $commandLibPhp);
    flus_assert_contains("LIMIT 1 FOR UPDATE", $commandLibPhp);
    flus_assert_contains("\$productUid !== 'id:' . \$productId", $commandLibPhp);
    flus_assert_contains("'PRICE_CHANGED_LOCALLY'", $commandLibPhp);
    flus_assert_contains('precio_registrar_cambio(', $commandLibPhp);
    flus_assert_contains('flus_cloud_sync_enqueue_stock_snapshot($pdo, [$productId], \'command\')', $commandLibPhp);
    flus_assert_contains("flus_cloud_command_complete_receipt(\$pdo, \$receiptId, 'applied', \$result)", $commandLibPhp);
    flus_assert_not_contains('FLUS_LICENSE_CLOUD_TOKEN', $commandLibPhp);
    ...
    require_once $repoRoot . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR . 'cloud_command_lib.php';
    flus_assert_same('https://api.flus.com.ar/command-poll.php', flus_cloud_command_endpoint('https://api.flus.com.ar/sync-ingest.php', 'command-poll.php'));
    flus_assert_same('https://panel.example.com/admin/api/command-ack.php', flus_cloud_command_endpoint('https://panel.example.com/admin/api/sync-ingest.php', 'command-ack.php'));
    ...
    require_once $repoRoot . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR . 'cloud_sync_sales_backfill_lib.php';
    flus_assert_same(500, flus_cloud_sync_sales_backfill_normalize_options(['limit' => 999])['limit']);
    flus_assert_same(0, flus_cloud_sync_sales_backfill_normalize_options(['after_id' => -5])['after_id']);
    $invalidRangeBlocked = false;
    try {
        flus_cloud_sync_sales_backfill_normalize_options(['from' => '2026-07-30', 'to' => '2026-07-29']);
    } catch (InvalidArgumentException $e) {
        $invalidRangeBlocked = $e->getMessage() === 'DATE_RANGE_INVALID';
    }
    flus_assert_true($invalidRangeBlocked);
});

$results[] = flus_run_test('instalador 4.2.10 preserva datos y exige backup previo', function (): void {
    $repoRoot = dirname(__DIR__);
    $installerRoot = $repoRoot . DIRECTORY_SEPARATOR . 'installer';
    if (!is_dir($installerRoot)) {
        flus_skip('instalador 4.2.10 preserva datos y exige backup previo: fuentes de build excluidas del payload productivo');
    }
    ...
    flus_assert_contains('#define MyAppVersion "4.2.10"', $serverIss);
...
$results[] = flus_run_test('caja mantiene un unico resumen operativo de cobro', function (): void {
    ...
    $cajaPhp = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'caja.php');
    $cajaJs = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'js' . DIRECTORY_SEPARATOR . 'caja.js');
    $cajaMovimientosModalJs = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'js' . DIRECTORY_SEPARATOR . 'caja_movimientos_modal.js');
    $cajaPosCss = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'css' . DIRECTORY_SEPARATOR . 'caja.pos.css');
    $cajaMovimientosPhp = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'caja_movimientos.php');
    ...
    $cajaSesionDetallePhp = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'caja_sesion_detalle.php');
    $cajaSesionDetalleJs = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'js' . DIRECTORY_SEPARATOR . 'caja_sesion_detalle.js');
    $cajaSesionDetalleCss = (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'css' . DIRECTORY_SEPARATOR . 'caja_sesion_detalle.css');
    ...
    flus_assert_contains('class="alert alert-success caja-flash"', $cajaPhp);
    flus_assert_contains('class="alert alert-error caja-flash"', $cajaPhp);
    flus_assert_contains('id="denomChips" class="denom-chips is-hidden"', $cajaPhp);
    flus_assert_contains('id="btnMovimientosCaja"', $cajaPhp);
    flus_assert_contains('id="cajaMovimientoModal" class="caja-movimiento-modal hidden"', $cajaPhp);
    flus_assert_contains('id="cajaMovimientoForm" class="caja-movimiento-form" method="post" action="caja_movimientos.php"', $cajaPhp);
    flus_assert_contains('name="response" value="json"', $cajaPhp);
    flus_assert_contains('id="cajaMovimientoRequestUid"', $cajaPhp);
    flus_assert_contains('id="cajaMovimientoHistoryToggle"', $cajaPhp);
    flus_assert_contains('id="cajaMovimientoHistoryList"', $cajaPhp);
    flus_assert_contains('<?php if ($canVerControlMovimientos): ?>', $cajaPhp);
    flus_assert_contains('assets/js/caja_movimientos_modal.js?v=', $cajaPhp);
    flus_assert_contains('const btn = document.getElementById("btnMovimientosCaja");', $cajaMovimientosModalJs);
    flus_assert_contains('const historyToggle = document.getElementById("cajaMovimientoHistoryToggle");', $cajaMovimientosModalJs);
    flus_assert_contains('btn.addEventListener("click", openModal);', $cajaMovimientosModalJs);
    flus_assert_contains('historyToggle?.addEventListener("click", loadHistory);', $cajaMovimientosModalJs);
    flus_assert_contains('url.searchParams.set("response", "json");', $cajaMovimientosModalJs);
    flus_assert_contains('function renderHistory(items)', $cajaMovimientosModalJs);
    flus_assert_contains('body: new FormData(form)', $cajaMovimientosModalJs);
    flus_assert_contains('"X-Requested-With": "XMLHttpRequest"', $cajaMovimientosModalJs);
    flus_assert_contains('!isElementHidden(document.getElementById("cajaMovimientoModal"))', $cajaJs);
    flus_assert_contains('chips.classList.toggle("is-hidden", !targets.length);', $cajaJs);
    flus_assert_contains('caja-notif-money caja-notif-money--success', $cajaJs);
    ...
    flus_assert_contains('if ($canVerControlMovimientos) {', $cajaMovimientosPhp);
    flus_assert_contains('<?php if ($canVerControlMovimientos): ?>', $cajaMovimientosPhp);
    flus_assert_contains('class="mov-control-note"', $cajaMovimientosPhp);
    flus_assert_contains('.mov-error {', $cajaMovimientosCss);
    flus_assert_contains('.mov-control-note {', $cajaMovimientosCss);
    flus_assert_contains('.caja-movimiento-modal {', (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'css' . DIRECTORY_SEPARATOR . 'caja.base.css'));
    flus_assert_contains('.caja-movimiento-type input:checked + span', (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'css' . DIRECTORY_SEPARATOR . 'caja.base.css'));
    flus_assert_contains('.caja-movimiento-history__item', (string)file_get_contents($repoRoot . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'css' . DIRECTORY_SEPARATOR . 'caja.base.css'));
    flus_assert_contains('class="empty-message filter-empty is-hidden"', $cajaSesionDetallePhp);
    flus_assert_contains('fila.classList.toggle(\'is-hidden\', !mostrar);', $cajaSesionDetalleJs);
    flus_assert_contains('noResultsMsg.classList.toggle(\'is-hidden\', visibles !== 0);', $cajaSesionDetalleJs);
    flus_assert_contains('.flus-tooltip {', $cajaSesionDetalleCss);
    flus_assert_contains('@keyframes cajaTooltipFadeIn', $cajaSesionDetalleCss);
    flus_assert_not_contains('id="noResultsMsg" style="display: none;"', $cajaSesionDetallePhp);
    flus_assert_not_contains('fila.style.display', $cajaSesionDetalleJs);
    flus_assert_not_contains('noResultsMsg.style.display', $cajaSesionDetalleJs);
    flus_assert_not_contains('tooltip.style.cssText', $cajaSesionDetalleJs);
    flus_assert_not_contains("document.createElement('style')", $cajaSesionDetalleJs);
    flus_assert_not_contains('style="margin-bottom:12px;"', $cajaPhp);
    flus_assert_not_contains('style="margin-bottom:12px;"', $cajaMovimientosPhp);
    flus_assert_not_contains('class="caja-mov"', $cajaPhp);
    flus_assert_not_contains('data-caja-summary-action', $cajaPhp);
    flus_assert_not_contains('document.querySelectorAll("[data-caja-summary-action]")', $cajaJs);
    flus_assert_not_contains('$movIngresos', $cajaPhp);
    flus_assert_not_contains('$movEgresos', $cajaPhp);
    flus_assert_not_contains('onclick="event.preventDefault(); event.stopPropagation(); window.location=\'caja_movimientos.php\';"', $cajaPhp);
    flus_assert_not_contains('id="denomChips" class="denom-chips" aria-label="Billetes rapidos" style="display:none"', $cajaPhp);
    flus_assert_not_contains('chips.style.display = targets.length ? "flex" : "none";', $cajaJs);
    flus_assert_not_contains('strong style="color:var(--accent-green,#22c55e)"', $cajaJs);
    flus_assert_not_contains('<p style="margin:6px 0">', $cajaJs);
    flus_assert_not_contains('style="color:var(--muted,#94a3b8);font-size:.88rem;margin-top:', $cajaJs);
