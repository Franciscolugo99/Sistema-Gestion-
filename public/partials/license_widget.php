...
if (!function_exists('flus_license_widget')) {
  function flus_license_widget(): string {
    ...
    $html .= '<div style="opacity:.8;">Vence</div><div>' . htmlspecialchars((string)$expStr, ENT_QUOTES, 'UTF-8') . '</div>';
    $html .= '<div style="opacity:.8;">Días restantes</div><div>' . htmlspecialchars((string)$dias, ENT_QUOTES, 'UTF-8') . '</div>';
    $html .= '</div>';
    $html .= '</div>';
