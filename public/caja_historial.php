...
if (!function_exists('render_pagination')) {
  // Paginación numérica (mismo estilo que Ventas/Stock)
  function render_pagination(int $page, int $totalPages, array $params, bool $showInfo = true, int $total = 0, int $from = 0, int $to = 0): string {
    ...
    if ($showInfo) {
      if ($total > 0) {
        ...
        $html .= '<span class="pagination-info">0-0 de 0</span>';
      }
    }
    ...
    for ($i = $start; $i <= $end; $i++) {
      if ($i === $page) {
        $html .= '<span class="pg-btn active">' . $i . '</span>';
      } else {
        $params['page'] = $i;
        $html .= '<a href="?' . http_build_query($params) . '" class="pg-btn">' . $i . '</a>';
      }
    }
